...
static const struct bios_settings_struct bios_settings[SETTINGS_AMOUNT] =
{
	...
	{OPTION_EMPTY, "", ""},
	{OPTION_ROM_CACHEABLE, "ROM Cacheable in CPU L1", "ROM_CACHEABLE"},
	{OPTION_RAM_CACHEABLE, "RAM Cacheable in CPU L1", "RAM_CACHEABLE"},
	...
	for(int i=0; i<SETTINGS_AMOUNT; i++) {
		...
		switch(bios_settings[i].setting) {
			...
			case OPTION_LOCK_CMOS:
				...
				break;
			case OPTION_ROM_CACHEABLE:
				draw_type = DRAW_YES_NO;
				draw_value[0] = cmos_get(CMOS_ROM_CACHEABLE) ? 1 : 0;
				break;
			case OPTION_RAM_CACHEABLE:
				draw_type = DRAW_YES_NO;
				draw_value[0] = cmos_get(CMOS_RAM_CACHEABLE) ? 1 : 0;
				break;
			...
			if(!extended) {
				...
				if(scancode==0x1C) { // Enter
					...
					switch(bios_settings[select].setting) {
						...
						case OPTION_ROM_CACHEABLE:
							OPTION_TYPE = OPTION_TYPE_YESNO;
							option_value[0] = cmos_get(CMOS_ROM_CACHEABLE);
							break;
						case OPTION_RAM_CACHEABLE:
							OPTION_TYPE = OPTION_TYPE_YESNO;
							option_value[0] = cmos_get(CMOS_RAM_CACHEABLE);
							break;
						case OPTION_OPEN_ABOUT:
					...
					switch(bios_settings[select].setting) {
						...
						case OPTION_LOCK_CMOS:
							...
							break;
						case OPTION_ROM_CACHEABLE:
							cmos_set(CMOS_ROM_CACHEABLE, option_value[0]);
							break;
						case OPTION_RAM_CACHEABLE:
							cmos_set(CMOS_RAM_CACHEABLE, option_value[0]);
							break;
						case OPTION_OPEN_ABOUT:
							about_display();
							break;
